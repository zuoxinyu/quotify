...
impl AugmentProvider {
    ...
    pub fn has_cli_or_token(enabled_token: &str) -> bool {
        !enabled_token.trim().is_empty()
            || std::env::var("AUGMENT_SESSION_TOKEN")
                .ok()
...
impl Provider for AugmentProvider {
    ...
    async fn fetch_usage(&self) -> Result<UsageData> {
        let token = if !self.enabled_token.trim().is_empty() {
        ...
        let output = tokio::task::spawn_blocking(move || run_auggie_usage(token.as_deref()))
            .await
            .context("Failed to join Augment usage task")??;
        let windows = parse_usage_windows(&output);
        if windows.is_empty() {
            anyhow::bail!("Augment usage output did not contain parseable quota data: {output}");
        }
...
fn run_auggie_usage(token: Option<&str>) -> Result<String> {
    ...
    if let Some(token) = token {
        cmd.env("AUGMENT_SESSION_TOKEN", token);
    }
    ...
    let mut child = cmd.spawn().context("Failed to run auggie usage")?;
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        if child
            .try_wait()
            .context("Failed to poll auggie usage")?
            .is_some()
        {
            break;
        }
        if Instant::now() >= deadline {
            let _ = child.kill();
            let _ = child.wait();
            anyhow::bail!("auggie usage timed out after 10s");
        }
    ...
    let output = child
        .wait_with_output()
        .context("Failed to collect auggie usage output")?;
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        anyhow::bail!(
            "auggie usage exited with {}: {}",
            output.status,
            stderr.trim()
        );
    }
