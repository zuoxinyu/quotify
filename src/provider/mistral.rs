    ...
    fn base_url(&self) -> String {
        if !self.base_url.trim().is_empty() {
            ...
            std::env::var("MISTRAL_API_BASE")
                .or_else(|_| std::env::var("MISTRAL_BASE_URL"))
                .ok()
...
impl Provider for MistralProvider {
    ...
    async fn fetch_usage(&self) -> Result<UsageData> {
        let api_key = self
            .resolve_api_key()
            .context("Mistral API key not configured. Set api_key or MISTRAL_API_KEY")?;
        ...
        let resp = self
            ...
            .send()
            .await
            .context("Failed to connect to Mistral models API")?;
        if !resp.status().is_success() {
            let status = resp.status();
            let body = resp.text().await.unwrap_or_default();
            anyhow::bail!("Mistral models API error: {status} - {body}");
        }
