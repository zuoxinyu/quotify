...
impl GeminiProvider {
    ...
    fn read_oauth_credentials() -> Option<GeminiCredentials> {
...
impl Provider for GeminiProvider {
    ...
    async fn fetch_usage(&self) -> Result<UsageData> {
        ...
        {
            anyhow::bail!(
                "Gemini usage requires Gemini CLI OAuth login; API key and Vertex modes do not expose quota usage"
            );
        }

        let credentials = Self::read_oauth_credentials().with_context(|| {
            if self.has_configured_api_key() {
                "Gemini API key is configured, but usage quota requires Gemini CLI OAuth credentials at ~/.gemini/oauth_creds.json"
            } else {
                "Gemini OAuth credentials not found. Run Gemini CLI OAuth login first"
            }
        })?;
        ...
        let quota = match self.fetch_quota_with_token(&access_token).await {
            ...
            Err(e) if is_auth_error(&e) => {
                tracing::debug!("Gemini quota token rejected, refreshing and retrying: {e}");
                let refresh_token = credentials
                    .refresh_token
                    .clone()
                    .context("Gemini OAuth token was rejected and no refresh token is available")?;
                let refreshed = self
                    .refresh_access_token(credentials, &refresh_token)
                    .await
                    .context("Gemini OAuth token was rejected and refresh failed")?;
                self.fetch_quota_with_token(&refreshed).await?
            }
            Err(e) => return Err(e),
        };
        let windows = parse_gemini_quota(&quota);

        if windows.is_empty() {
            anyhow::bail!("Gemini quota response did not contain model usage data");
        }
...
impl GeminiProvider {
    async fn fetch_quota_with_token(&self, access_token: &str) -> Result<serde_json::Value> {
        let project = match self.load_code_assist_project(access_token).await {
            Ok(project) => project,
            Err(e) => {
                tracing::debug!("Gemini loadCodeAssist project discovery failed: {e}");
    ...
    async fn resolve_access_token(&self, credentials: GeminiCredentials) -> Result<String> {
        ...
        match self.refresh_access_token(credentials, &refresh_token).await {
            Ok(token) => Ok(token),
            Err(e) => {
                tracing::debug!("Gemini OAuth token refresh failed: {e}");
    ...
    async fn refresh_access_token(
        &self,
        mut credentials: GeminiCredentials,
        refresh_token: &str,
    ) -> Result<String> {
        let (client_id, client_secret) =
            find_gemini_oauth_client().context("Gemini CLI OAuth client not found")?;
        ...
        if !resp.status().is_success() {
            ...
            anyhow::bail!("Gemini OAuth refresh failed: {status} - {body}");
        }

        let refreshed: serde_json::Value = resp
            .json()
            .await
            .context("Failed to parse Gemini OAuth refresh response")?;
        let access_token = string_field(&refreshed, &["access_token", "accessToken"])
            .context("Gemini OAuth refresh response missing access token")?;
        ...
        .with_context(|| {
            format!(
                "Failed to write refreshed Gemini credentials to {:?}",
                credentials.path
            )
        })?;
    ...
    async fn load_code_assist_project(&self, access_token: &str) -> Result<Option<String>> {
        let resp = self
            ...
            .send()
            .await
            .context("Failed to call Gemini loadCodeAssist")?;

        if !resp.status().is_success() {
            let status = resp.status();
            let body = resp.text().await.unwrap_or_default();
            anyhow::bail!("Gemini loadCodeAssist error: {status} - {body}");
        }
    ...
    async fn retrieve_user_quota(
        ...
        let resp = self
            ...
            .send()
            .await
            .context("Failed to call Gemini quota API")?;

        if !resp.status().is_success() {
            let status = resp.status();
            let body = resp.text().await.unwrap_or_default();
            anyhow::bail!("Gemini quota API error: {status} - {body}");
        }
...
fn select_gemini_project(value: &serde_json::Value) -> Option<String> {
...
fn find_gemini_oauth_client() -> Option<(String, String)> {
...
fn parse_gemini_quota(value: &serde_json::Value) -> Vec<UsageWindow> {
    ...
    for (label, needle) in groups {
        if let Some(window) = best_gemini_window(
            label,
            entries
                .iter()
...
fn collect_quota_entries(
    ...
    match value {
        serde_json::Value::Object(map) => {
            ...
            .or_else(|| {
                model_hint
                    .filter(|hint| hint.to_ascii_lowercase().contains("gemini"))
                    .map(ToString::to_string)
            });
...
fn best_gemini_window<'a>(
    label: &str,
    entries: impl Iterator<Item = &'a GeminiQuotaEntry>,
) -> Option<UsageWindow> {
