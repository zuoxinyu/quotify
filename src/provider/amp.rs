...
const DEFAULT_URL: &str = "https://ampcode.com/settings";
...
impl Provider for AmpProvider {
    ...
    async fn fetch_usage(&self) -> Result<UsageData> {
        let cookie = self
            .resolve_cookie()
            .context("Amp cookie not configured. Set api_key, AMP_COOKIE, or AMPCODE_COOKIE")?;
        ...
        let resp = self
            ...
            .send()
            .await
            .context("Failed to connect to Amp settings page")?;
        if !resp.status().is_success() {
            let status = resp.status();
            let body = resp.text().await.unwrap_or_default();
            anyhow::bail!("Amp settings page error: {status} - {body}");
        }

        let text = resp
            .text()
            .await
            .context("Failed to read Amp settings page")?;
        let windows = parse_usage(&text);
        if windows.is_empty() {
            anyhow::bail!("Amp settings page did not contain parseable usage data");
        }
...
fn parse_usage(html: &str) -> Vec<UsageWindow> {
    ...
    let fraction = Regex::new(
        r"(?i)(Amp Free|Free|Usage|Credits).{0,80}?(\d+(?:\.\d+)?)\s*/\s*(\d+(?:\.\d+)?)",
    )
    .unwrap();
    if let Some(caps) = fraction.captures(&text) {
        let used = caps[2].parse::<f64>().unwrap_or(0.0);
        let limit = caps[3].parse::<f64>().unwrap_or(0.0);
        return vec![UsageWindow {
    ...
    let percent =
        Regex::new(r"(?i)(Amp Free|Free|Usage|Credits).{0,80}?(\d+(?:\.\d+)?)\s*%").unwrap();
    percent
        .captures(&text)
        .map(|caps| {
            vec![UsageWindow {
                label: caps[1].trim().to_string(),
                used_percent: caps[2].parse::<f64>().unwrap_or(0.0).clamp(0.0, 100.0),
                limit: None,
                used: None,
                unit: None,
                resets_at: None,
